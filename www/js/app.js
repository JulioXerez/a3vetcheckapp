document.addEventListener('DOMContentLoaded', async function(){
  ...
  async function refreshTodayAgenda(){
    const rows = await Appointments.listAllAppointments();
    const pets = await Pets.listAllPets();
    // map pet id -> pet
    const petMap = {};
    pets.forEach(p=>{ petMap[p.id] = p; });
    // map users id -> name for pet owners
    let users = [];
    try{ users = await DB.query('SELECT id, name FROM users', []); }catch(e){ users = []; }
    const userMap = {};
    ...
    const todays = (rows||[]).filter(r => (r.date||'').startsWith(todayIso));
    if(todays.length===0){
      const li = document.createElement('li'); li.textContent = 'Nenhuma consulta hoje.'; li.style.opacity = 0.8; container.appendChild(li); return;
    }
    // sort by time
    todays.sort((a,b)=> (a.time||'').localeCompare(b.time||''));
    // find next upcoming appointment (today, >= now)
    const now = new Date();
    let nextApptId = null;
    const future = todays.filter(r => new Date((r.date||'') + 'T' + (r.time||'00:00')) >= now);
    ...
    todays.forEach(r=>{
      ...
      const ownerName = pet ? (userMap[pet.owner_id] || '') : '';
      const time = r.time || '';
      const vet = r.vet_name || '—';
      // show: time — pet (owner) • type • vet
      li.innerHTML = `<strong>${time}</strong> — ${petName}` + (ownerName? ` <em style="color:#6b7280">(${ownerName})</em>`:'') + (r.type ? ` <span style="color:#6b7280">• ${r.type}</span>` : '') + ` <span style="color:#6b7280">• ${vet}</span>`;
  ...
  async function renderWeek(date){
    ...
    if(gridEl){
      ...
      (rows||[]).forEach(r=>{
        ...
        div.innerHTML = `<div style="font-weight:700">${petName}</div><div class="meta">${r.time||''} • ${r.vet_name||''}${r.type ? ' • ' + r.type : ''}${r.duration ? ' • ' + r.duration + 'm' : ''}</div>`;
  ...
  const btnPrev = document.getElementById('weekPrev'); if(btnPrev) btnPrev.addEventListener('click', async ()=>{ if(!currentWeekStart) currentWeekStart = startOfWeek(new Date()); currentWeekStart.setDate(currentWeekStart.getDate()-7); await renderWeek(currentWeekStart); });
  const btnNext = document.getElementById('weekNext'); if(btnNext) btnNext.addEventListener('click', async ()=>{ if(!currentWeekStart) currentWeekStart = startOfWeek(new Date()); currentWeekStart.setDate(currentWeekStart.getDate()+7); await renderWeek(currentWeekStart); });
  const btnToday = document.getElementById('todayBtn'); if(btnToday) btnToday.addEventListener('click', async ()=>{ await renderWeek(new Date()); });
  const btnRefresh = document.getElementById('refreshBtn'); if(btnRefresh) btnRefresh.addEventListener('click', async ()=>{ await renderWeek(currentWeekStart||new Date()); });
  ...
  // explicit save client button (if present)
  const btnSaveClient = document.getElementById('btnSaveClient');
  if(btnSaveClient) btnSaveClient.addEventListener('click', async function(){
    try{
      ...
      if(cpf && !isValidCPF(cpf)){ setCPFError('CPF inválido. Corrija antes de salvar.'); const cpfEl = document.getElementById('regCpf'); if(cpfEl) cpfEl.focus(); return; }
      const address = document.getElementById('regAddress') ? document.getElementById('regAddress').value.trim() : '';
      let cur = Auth.currentUser();
      if(!cur){
        if(!email){ alert('Informe o e-mail do cliente para cadastrar.'); return; }
        cur = await Auth.register(email, undefined, name, phone, address, cpf);
      } else {
        const newEmail = document.getElementById('regEmail') ? document.getElementById('regEmail').value.trim() : null;
        await Auth.updateProfile(cur.id, name, phone, address, cpf, newEmail || undefined);
      }
      if(typeof window.app !== 'undefined' && typeof window.app.refreshPets === 'function') await window.app.refreshPets();
      if(typeof window.app !== 'undefined' && typeof window.app.refreshAppts === 'function') await window.app.refreshAppts();
      setCPFError('');
      alert('Dados do cliente salvos.');
    }catch(e){ console.error('Salvar cliente erro', e); alert(e.message || String(e)); }
  });
  ...
  async function populateAppointmentEditor(appt){
    try{
      ...
      // focus the form area
      try{ const target = document.getElementById('chiefComplaint') || document.getElementById('apptsList'); if(target){ target.scrollIntoView({behavior:'smooth', block:'center'}); if(typeof target.focus==='function') target.focus(); } }catch(e){}
    }catch(e){ console.warn('populate editor error', e); }
  }

  // initial loads
  if (Auth.currentUser()){ await refreshPets(); await refreshAppts(); await refreshTodayAgenda(); }
});
